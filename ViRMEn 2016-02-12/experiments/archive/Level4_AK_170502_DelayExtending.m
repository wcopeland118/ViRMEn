function code = Level4_AK_170502_DelayExtending
...
%% trial record
vr.trialCount = 0;
vr.trialRecord = struct('cueType',[],'mouseTurn',[],'success',[],'block',vr.block);
...
switch vr.STATE
    case 'INIT_TRIAL'
        ...
        %vr.blockTrials = sum([vr.trialRecord.block]>=vr.trialBlockThresh);
        %vr.blockSuccessCount = sum([vr.trialRecord.success==1
        
        %hmm this is pretty not accurate. probably easier to do a live
        %count
        ...
        %     vr.worlds{1}.surface.visible(vr.greyTowers) = 1;
        % end
        vr.trialCount = vr.trialCount+1; %increment trial counters
        vr.trialStart = tic;
        vr.trialRecord(vr.trialCount).cueType=vr.cuePos;
        vr.STATE = 'TRIAL';
    case 'TRIAL'
        ...
        if abs(vr.position(1)) > eval(vr.exper.variables.armLength)/vr.armFac
            ...
            if vr.position(1) < 0 %left turn
                vr.trialRecord(vr.trialCount).mouseTurn='Left'
                if ismember(vr.cuePos,[1 3]) %correct L
                    vr.isReward=1;
                    vr.trialRecord(vr.trialCount).success=1;
                    vr=giveReward_AK(vr,vr.numRewPer);
                    
                    vr.itiDur = vr.itiCorrect;
                    vr.numRewards = vr.numRewards + 1;
                    vr.trialResults(1,size(vr.trialResults,2)+1) = 1; %WHAT?
                    vr.streak = vr.streak + 1;
                    vr.netCount = vr.netCount + 1;
                    vr.missFlag = false;
                    if vr.verbose
                        disp('Correct Left Turn Detected');
                    end
                elseif ismember(vr.cuePos,[2 4]) %incorrect L
                    vr.trialRecord(vr.trialCount).success=0;
                    vr.isReward = 0;
            ...
            elseif  vr.position(1) > 0 %R turn
                vr.trialRecord(vr.trialCount).mouseTurn='Right'
                if ismember(vr.cuePos,[1,3]) %incorrect R
                    ...
                    vr.netCount = vr.netCount - 1;
                    if vr.verbose
                        disp('Wrong Right Turn Detected');
                    end
                elseif ismember(vr.cuePos,[2 4]) %correct R
                    vr.isReward=1;
                    vr.trialRecord(vr.trialNum).success=1;
                    vr=giveReward_AK(vr,vr.numRewPer);
